...
//className="h-screen flex justify-center relative items-center snap-center perspective-600"
  ...
  <section style={{background: color }} className={`h-screen flex justify-center relative items-center snap-center perspective-1200`} >
    ...
    <motion.div ref={ref} animate={{x: hover ? -250 : 0, width: hover ? 500 : 300, height: hover ? 500 : 300}} className=' rounded-md w-[500px] h-[500px] relative m-[20px] overflow-hidden'>
      <motion.div  className='group-hover:rounded group-hover:shadow-md rounded-lg duration-500 absolute top-0 left-0 right-0 bottom-0 object-contain' ><Image src={`./Portfolio/${bilde}.jpg`}  alt='Project'></Image> </motion.div>
    </motion.div>
    ...
    <motion.a href={`${link}`} initial={{opacity: 0}} style={{y}} animate={{opacity: hover ? 100 : 0, x: hover ? 0 : 200}} transition={{delay: hover ? 0.5 : 0}} className= "left-[calc(50%+130px)] bg-blue-400 rounded shadow-md m-0 absolute top-[80%]"> <button className='w-40 text-Bcolor hover:scale-105 duration-100 font-semibold h-10'>Read more</button> </motion.a>
...
export default function Project() {
  ...
  {id: "Skiltsøkeren", color: "#151b23", beskrivelse: "Mobile application made with Kotlin. The application is a simple License plate runner that lets you scan with your camera or search for a license plate registered in Norway. ", bilde: "Pacman", link: "Java",},
  {id: "DataGenerator", color: "#151b23", beskrivelse: "Bachelor project made for Visma AS. The project has its own API and Blazor website. Application generates random fake data for existing data in a Database while still keeping real keys. ", bilde: "Pacman", link: "Java",},
  {id: "Paint", color: "#151b23", beskrivelse: "Simple paint application made with Java. The application lets you draw and use different tools to create an image. The app has fill, color, undo, redo, clear and layers. ", bilde: "Pacman", link: "Java",},
  ]
  return (
    <>
